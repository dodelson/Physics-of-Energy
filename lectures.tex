...
\eei
The first fact leads to the notion that each nucleon feels an attractive force from its neighbors but must maintain a distance of about 1 fm, so the density is constant, meaning that $V(A)\propto R^3(A) \propto A$. 
\bee
\item The binding energy then for each nucleon is the sum of the local forces, so the total binding energy picks up a term \
\be
B_V=\epsilon_VA
\ee
