...
\eei
\item Generalizing to decays, the prefactor must have dimensions of time (typically it is estimated to be the travel time in the potential, but it is dominated by the exponential.
\item Bottom line: the binding energy is akin to $V_0-E$ and the length of the barrier separating the spherical state to the (lower energy) elongated state is $L$. Roughly the decay rate scales as $e^{-2\sqrt{mb}L/\hbar}$.
\eee

\lsection{Impact of neutrons}

Neutron absorption transforms it into spontaneous decay.
