...
\lecture{Nuclear Decays and Reactors}
...
\item Work through Details of Reactor
\eei
...
\item Bottom line: the difference between the potential at its peak and $Q$ is akin to $V_0-E$ and the length of the barrier separating the spherical state to the (lower energy) elongated state is $L$. 

Very rough estimate: take the height to be roughly $Z\alpha\hbar c/R$; the mass equal to half the mass of the nucleus; and the distance
 If we estimate the barrier as the Coulomb barrier, so that 
\be
V_C(r) \sim\,\frac{Z \hbar c\alpha}{r}
\ee
then the potential peaks at $R\simeq 10^{-14}$m inside large nuclei (the strong force dominates interior to that) so can be as large as $E_B=20$MeV. In actuality, the barrier heights are a bit smaller.  The length is determined by the value of $r$ when the Coulomb force is equal to the energy released $Q$:
\be
 \frac{Z \alpha\hbar c}{L} = Q.\ee
 For $Q=200$ MeV, this sets $L\simeq 10^{-15}$m up to 100 times larger than that if the full Coulomb charge is felt.
 So a rough estimate for the argument of the exponential is, taking $E_B\sim 5$MeV and $L\sim 10^{-14}$m, 
 \bea
 \frac2{\hbar}\,\sqrt{2(M/2) E_B} \, L
...
\item Slow neutrons are better: the cross section is $10^4$ barns at $E=10^{-4}$ eV and about 100 times lower at 1 eV, so roughly it scales as $E^{-1/2}$. roughly speaking, the increase is because the cross section scales as $R^2$ and the Compton radius of the neutron is $h/p=2\pi\hbar/\sqrt{2mE}$.
\eei
...
\be
\eta = 2.42\,\left( \frac{x\sigma_f(^{235}U)}{x\sigma_a(^{235}U) + (1-x)\sigma_a(^{238}U)}
\right)
.\ee
\Sfig{f191}{This governs $\eta$, roughly equal to $B/A$.}
This assumes that the rate for neutron capture to fission is $n_U\sigma v$, which follows from considering a box of length $vdt$ and total area $A$. The number of $U$ atoms in that box is $nAvdt$; they each have a cross section $\sigma$, so the chance of being absorbed is $nAvdt \times \sigma/A$. Then the factors follows by the definition of $x\equiv 235/238$.
...
\bee
\item For wind speed $v_s$ due North ($\hat y$), the ocean velocity is
...
\item Air circulation globally is in hadley and polar cells. The former have warm air rising from the ocean near the equator towards the tropopause, moving northward (perhaps adiabatically following lines of constant potential?), and then dropping back to surface at latitudes of order 30$^\circ$ because it is now colder and drier. The air is then driven along the surface of the ocean back to the equator.
...
\item There is vorticity so wind blows north on the western side and south on the eatern side (p. 524). 
\item This produces ocean currents that flow southward towards the equator broadly and then return in narrow channels along the west ocean bordering land: the Gulf Stream in the Atlantic off the east coast of North America and the Kurishio in the Pacific along the Eastern coast of Asia.
\item Potential resource: eqn 27.11:
\be
100 m\,\times A \times 20K\times 4MJ/m^3/K = 5.5\times 10^{23} J\ee
...
\comment{This should be done more carefully, deriving 3.29 in Wallace and Hobbs.} Generally, we have
\be
\int_{P_0}^P\frac{dP'}{P'} = -\frac{g}{R}\, \int_0^z \frac{dz'}{T(z')}.\ee
The two limiting possibilities are isothermal and adiabatic:
\be
P^{iso} = P_0 e^{-z/H}\ee
with $H\equiv g/RT_0=\frac{m_{\rm air}g}{k_BT}.$ 
