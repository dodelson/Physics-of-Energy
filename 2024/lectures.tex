...
\be
P^{iso} = P_0 e^{-z/H}\ee
with $H\equiv RT_0/g=\frac{k_BT}{m_{\rm air}g}.$ Note that $H=8.2$km when $T=283$.
%\item \be
...
\Spng{dpdt}{Difference in pressure as a function of height when the ground temperature differs by 10$^\circ$C. Both the adiabatic and isotwhen $T=283$.hermal limits are shown.}


\lsection{Circulation}
...
\item Hadley cells have warm air rising from the ocean near the equator towards the tropopause due to convection, moving northward (following lines of constant pressure), and then dropping back to surface at latitudes of order 30$^\circ$ because the ambient air is now colder and drier. 
